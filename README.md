...
- `checkpoint-920000.pt`: checkpoint mới nhất theo training step, đã tách phần inference và lưu FP16.
- `checkpoint-700000.pt`: checkpoint FP16 trước đó, được giữ để đối chiếu với bộ audio demo hiện tại.
...
Wrapper mặc định sẽ tự tìm các file `checkpoint-<step>.pt` và load checkpoint có step lớn nhất. Hiện tại checkpoint mới nhất là `checkpoint-920000.pt`.
